...
test('GitHub Pages workflow builds and deploys dist', () => {
  ...
  assert.match(yml, /npm run verify:quality/)
  const quality = read('scripts/run-quality-gate.mjs')
  assert.match(quality, /\['test'\]/)
  assert.match(quality, /\['run', 'test:unit'\]/)
  assert.match(quality, /\['run', 'typecheck'\]/)
...
test('workflow verifies clean source before generating runtime browser config', () => {
  ...
  const quality = yml.indexOf('Quality gate')
  ...
  assert.ok(quality >= 0, 'workflow must run the aggregate quality gate')
  assert.match(yml.slice(clean, quality), /rm -f public\/config\.js/)
  assert.ok(clean < quality, 'runtime config cleanup must happen before the quality gate')
  assert.ok(quality < generate, 'runtime config must be generated only after the quality gate')
