...
declare n public.homework_notices; cand jsonb; score numeric; candidate uuid; st public.homework_settings; final_state text; old jsonb;begin
 ...
 if exists(select 1 from public.homework_duplicate_reviews where notice_id=n.id and revision=n.revision)then return '{"done":true}';end if;
 ...
 insert into public.homework_duplicate_reviews(notice_id,revision,reason)values(n.id,n.revision,'Tài khoản hoặc lớp đã ngừng hoạt động trong lúc AI xử lý.');
 ...
 insert into public.homework_duplicate_reviews(notice_id,revision,reason)values(n.id,n.revision,'AI tạm thời không xử lý được; bài tiếp tục chờ kết quả so sánh hợp lệ.');
 ...
 insert into public.homework_duplicate_reviews(notice_id,revision,candidate_id,score,reason)values(n.id,n.revision,candidate,score,left(p_data->>'reason',2000));
