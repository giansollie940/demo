...
test('R6.5 dashboard hero artwork fills the hero height and blends from the right edge', () => {
  const page = read('src/pages/DashboardPage.vue')
  assert.match(page, /\.dashboard-hero\{[^}]*min-height:190px[^}]*position:relative/s)
  assert.match(page, /\.hero-copy\{[^}]*position:relative[^}]*z-index:2[^}]*max-width:56%/s)
  assert.match(page, /\.hero-illustration\{[^}]*position:absolute[^}]*inset:0 0 0 auto[^}]*width:min\(52%,640px\)[^}]*height:100%/s)
  assert.match(page, /\.hero-illustration img\{[^}]*width:100%[^}]*height:100%[^}]*object-fit:cover/s)
...
test('R6.5 tablet layout explicitly returns hero artwork to document flow', () => {
  const page = read('src/pages/DashboardPage.vue')
  assert.match(page, /@media\(max-width:1100px\)[\s\S]*\.hero-copy\{[^}]*max-width:none/s)
  assert.match(page, /@media\(max-width:1100px\)[\s\S]*\.hero-illustration\{[^}]*position:relative[^}]*inset:auto[^}]*width:100%[^}]*height:160px/s)
})
...
test('R6.5 cleanup removes obsolete dashboard assets and legacy hero geometry', () => {
  ...
  assert.equal(fs.existsSync(path.join(root, 'public/assets/images/student-group-dashboard.png')), false)
  assert.doesNotMatch(page, /\.hero-illustration img\{[^}]*width:500px/s)
  assert.doesNotMatch(page, /\.hero-illustration img\{[^}]*object-fit:contain/s)
})
