...
test('desktop sidebar uses an edge chevron control while mobile keeps the hamburger menu',()=>{
  ...
  assert.match(shell,/\.sidebar-edge-toggle\s*\{[^}]*right:\s*-17px[^}]*border-radius:\s*(?:50%|999px)/s)
...
test('main surface uses an independent vanilla school-pattern layer above the warm base',()=>{
  ...
  assert.match(shell,/background-size:\s*(?:920|1100)px\s+auto/)
...
test('sidebar, icon buttons, and profile chip use modern hover micro-interactions',()=>{
  ...
  assert.match(nav,/translateY\(var\(--dock-shift-y\)\) translateX\(var\(--dock-lift-x\)\) scale\(var\(--dock-scale\)\)/)
  ...
  assert.match(shell,/\.sidebar-edge-toggle:hover\s*\{[^}]*scale\(1\.08\)/s)
})
test('dashboard R7 makes the integrated greeting hero primary while keeping week and class visible',()=>{
  const page=read('src/pages/DashboardPage.vue')
  const hero=read('src/components/dashboard/DashboardHero.vue')
  assert.match(page,/DashboardHero/)
  assert.match(page,/:week-label="weekLabel"/)
  assert.match(page,/:class-label="classLabel"/)
  assert.match(hero,/Xin chào \{\{ name \}\}!/)
  assert.match(hero,/class="hero-pills"/)
  assert.match(hero,/dashboard-hero-art/)
})
