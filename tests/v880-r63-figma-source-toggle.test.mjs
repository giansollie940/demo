...
test('R6.7 keeps the proven dashboard hierarchy while adopting the approved reference geometry',()=>{
  const page=read('src/pages/DashboardPage.vue')
  assert.match(page,/\.dashboard-page\{[^}]*max-width:1296px/s)
  assert.match(page,/\.dashboard-hero\{[^}]*min-height:132px/s)
  ...
  assert.match(page,/\.work-panel[^}]*min-height:340px/s)
  assert.match(page,/\.overview-panel[^}]*min-height:340px/s)
})
