...
async function call(action,data,review=false){return fetch('/__feat006/api',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({actor,action,data,review})}).then(r=>r.json());}
window.SupabaseService={init:async()=>({rpc:async(_,p)=>call(p.p_action,p.p_data),functions:{invoke:async(name,p)=>{const result=await call(p.body.action||'submit',p.body,name==='homework-review'?true:'media');return result.error?result:{data:name==='homework-media'?result.data:{ok:true,notice:result.data},error:null};}}})};
