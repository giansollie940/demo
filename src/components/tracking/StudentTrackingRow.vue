...
<template>
  <article class="tracking-row" :class="{busy}" :data-bucket="row.bucket">
    <header><div class="identity"><RemoteUserAvatar :user-id="row.user.id" :name="row.user.name" :code="row.user.code" size="md"/><div><h4>{{ row.user.name }}</h4><small>{{ row.user.code }}</small></div></div><span class="status">{{ statusText() }}</span></header>
    <div class="facts">
    ...
    <div v-if="busy" class="busy-indicator" role="status"><span class="diamond" aria-hidden="true"></span>Đang xử lý và đồng bộ…</div>
...
<style scoped>
.tracking-row{display:grid;gap:12px;border:1px solid var(--border);border-radius:16px;background:var(--surface-raised);padding:15px}.tracking-row.busy{border-color:color-mix(in srgb,var(--color-primary) 32%,var(--border));box-shadow:0 0 0 3px color-mix(in srgb,var(--color-primary) 7%,transparent)}.tracking-row[data-bucket="attention"]{border-color:color-mix(in srgb,var(--color-warning) 35%,var(--border))}.tracking-row header{display:flex;justify-content:space-between;gap:12px}.identity{display:flex;align-items:center;gap:10px}.identity h4{margin:0}.identity small{color:var(--text-muted)}.status{align-self:flex-start;border-radius:999px;padding:5px 9px;background:var(--surface-soft);color:var(--text-muted);font-size:var(--font-size-ui-min);font-weight:900}.facts{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.facts>span{display:grid;gap:3px;border-radius:12px;background:var(--surface-soft);padding:9px}.facts small,.content>small{color:var(--text-muted);font-size:var(--font-size-ui-min)}.facts b{display:flex;align-items:center;gap:5px;font-size:.84rem}.facts svg{width:14px}.facts .yes{color:var(--color-info)}.content p{margin:4px 0 0;white-space:pre-wrap}.content .note{color:var(--text-muted);font-size:.86rem}.teacher-comment,.missing-note,.attention-note{border-radius:11px;padding:9px 11px;background:var(--surface-soft);font-size:.83rem}.missing-note,.attention-note{display:flex;align-items:center;gap:7px;color:var(--text-muted)}.attention-note{color:var(--color-warning)}.missing-note svg,.attention-note svg{width:16px}.busy-indicator{display:flex;align-items:center;gap:8px;color:var(--color-primary);font-size:var(--font-size-ui-min);font-weight:850}.diamond{width:10px;height:10px;border-radius:2px;background:currentColor;transform:rotate(45deg);animation:busy-diamond .8s ease-in-out infinite}@keyframes busy-diamond{50%{transform:rotate(135deg) scale(.72);opacity:.55}}.actions{display:flex;gap:7px;flex-wrap:wrap}.actions :deep(.app-button){min-height:40px;padding:8px 11px;font-size:var(--font-size-ui-min)}.actions :deep(svg){width:15px}@media(max-width:680px){.facts{grid-template-columns:1fr}.actions :deep(.app-button){flex:1 1 calc(50% - 7px)}}
</style>
