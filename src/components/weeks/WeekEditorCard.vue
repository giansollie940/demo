...
import { AlertCircle, CalendarClock, CheckCircle2, Eye, Palmtree, Save } from 'lucide-vue-next'
  ...
  <article class="week-editor-card" :class="{ current, viewing }">
    ...
    <footer>
      <div v-if="showSaveBar" class="editor-save-bar" :class="`is-${saveState}`" :role="saveState==='error'?'alert':'status'">
        ...
        <AppButton :loading="saveState==='saving'" :disabled="disabled||!dirty" @click="emit('save')"><Save aria-hidden="true"/> Lưu thay đổi</AppButton>
      </div>
