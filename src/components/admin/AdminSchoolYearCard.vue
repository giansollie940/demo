...
import type { AdminCalendarWeekRecord, AdminSchoolYearRecord } from '../../features/admin/admin-directory'

const props=defineProps<{item:AdminSchoolYearRecord;weeks:AdminCalendarWeekRecord[];busy:boolean;busyWeekId?:string|null}>()
const emit=defineEmits<{activate:[id:string];saveWeek:[input:{weekId:string;startDate:string;endDate:string}]}>()
const drafts=reactive<Record<string,{startDate:string;endDate:string}>>({})

watch(()=>props.weeks,rows=>{
  for(const week of rows){
    const current=drafts[week.id]
    if(!current||current.startDate===week.startDate&&current.endDate===week.endDate){
      drafts[week.id]={startDate:week.startDate,endDate:week.endDate}
    }
  }
