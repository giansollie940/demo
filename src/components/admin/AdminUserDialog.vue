...
watch(()=>[props.open,props.kind,(props.user as {id?:string}|null)?.id] as const,()=>{
  ...
  <div v-if="open" class="backdrop" @click.self="emit('close')" @keydown.esc="emit('close')">
    <section class="dialog" role="dialog" aria-modal="true" aria-labelledby="adminUserDialogTitle">
      ...
      <form @submit.prevent="submit">
        ...
        <div class="form-grid">
          <label>Mã đăng nhập<input v-model="form.code" :disabled="editing&&!form.changeCode" maxlength="32" required></label>
          <label>Họ và tên<input v-model="form.fullName" maxlength="120" required></label>
          ...
          <label v-if="kind==='learner'">Lớp<select v-model="form.classId" required><option value="">Chọn lớp</option><option v-for="item in classes.filter(row=>row.active)" :key="item.id" :value="item.id">{{ item.code }} · {{ item.name }}</option></select></label>
