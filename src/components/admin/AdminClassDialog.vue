...
<style scoped>.dialog-backdrop{position:fixed;inset:0;z-index:130;display:grid;place-items:center;padding:18px;background:rgb(36 25 35/.45);backdrop-filter:blur(8px)}.dialog{width:min(480px,100%);display:grid;gap:14px;padding:20px;border:1px solid var(--border);border-radius:22px;background:var(--surface);box-shadow:0 30px 80px rgb(30 20 30/.28)}header,header>div,footer{display:flex;align-items:center;gap:10px}header{justify-content:space-between}header>div>svg{width:25px;color:var(--color-primary)}header span{display:grid}header small{color:var(--text-muted)}header button{border:0;background:transparent;color:var(--text-muted)}header button svg{width:20px}label{display:grid;gap:6px;font-size:.8rem;font-weight:800;color:var(--text-muted)}input{min-height:44px;border:1px solid var(--border);border-radius:11px;padding:8px 10px;background:var(--input);color:var(--text)}footer{justify-content:flex-end}.error{margin:0;color:var(--color-danger);font-weight:750}</style>
