...
function dockScale(index:number){
  ...
  if(distance===1)return props.collapsed?1.08:1.025
  return 1
}
</script>
...
<template>
  <nav class="side-nav" :class="{collapsed}" aria-label="Điều hướng chính" @mouseleave="hoveredIndex=null">
    <RouterLink
      ...
      :style="{'--dock-scale':String(dockScale(index))}"
...
.nav-item{--nav-accent:var(--color-primary);--dock-scale:1;position:relative;z-index:1;isolation:isolate;overflow:visible;min-height:48px;display:flex;align-items:center;gap:10px;padding:5px 11px 5px 6px;border:1px solid color-mix(in srgb,var(--nav-accent) 8%,var(--border));border-radius:17px;background:color-mix(in srgb,var(--surface) 72%,transparent);color:var(--text-muted);font-size:.91rem;font-weight:780;box-shadow:0 5px 14px rgb(79 55 73 / .035);transform:translate3d(0,0,0) scale(var(--dock-scale));transform-origin:left center;transition:background var(--transition-fast),color var(--transition-fast),transform .28s cubic-bezier(.2,1.5,.35,1),border-color var(--transition-fast),box-shadow .28s cubic-bezier(.2,1.5,.35,1);will-change:transform}
...
.nav-item:hover,.nav-item:focus-visible{z-index:8;background:linear-gradient(105deg,color-mix(in srgb,var(--wash-peach) 66%,var(--surface)),color-mix(in srgb,var(--nav-accent) 10%,var(--surface)));border-color:color-mix(in srgb,var(--nav-accent) 28%,var(--border));color:var(--nav-accent);transform:translate3d(4px,-3px,18px) scale(var(--dock-scale));box-shadow:0 16px 34px color-mix(in srgb,var(--nav-accent) 15%,transparent),0 5px 12px rgb(67 45 57 / .08)}
