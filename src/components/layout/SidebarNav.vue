...
.nav-item{--nav-accent:var(--color-primary);--dock-scale:1;--dock-shift-y:0px;--dock-lift-x:0px;--dock-gap-before:0px;--dock-gap-after:0px;position:relative;z-index:1;isolation:isolate;overflow:visible;min-height:48px;display:flex;align-items:center;gap:10px;margin-top:var(--dock-gap-before);margin-bottom:var(--dock-gap-after);padding:5px 11px 5px 6px;border:1px solid transparent;border-radius:14px;background:transparent;color:var(--text-muted);font-size:.84rem;font-weight:780;box-shadow:none;transform:translateY(var(--dock-shift-y)) translateX(var(--dock-lift-x)) scale(var(--dock-scale));transform-origin:left center;transition:background var(--transition-fast),color var(--transition-fast),transform .3s cubic-bezier(.2,1.5,.35,1),margin .3s cubic-bezier(.2,1.5,.35,1),border-color var(--transition-fast),box-shadow .3s cubic-bezier(.2,1.5,.35,1);will-change:transform,margin}
...
.nav-item.active{background:linear-gradient(135deg,#258cf8,#2877eb);border-color:color-mix(in srgb,var(--color-sky) 52%,transparent);color:#fff;box-shadow:0 10px 22px rgb(37 128 238 / .22)}
.nav-item.active .nav-icon-bubble{background:rgb(255 255 255 / .18);box-shadow:inset 0 0 0 1px rgb(255 255 255 / .22);color:#fff}
