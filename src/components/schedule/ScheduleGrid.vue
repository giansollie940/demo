    ...
    <div class="desktop-schedule" role="grid" aria-label="Thời khóa biểu tự học">
      ...
      <template v-for="period in periods" :key="period.n">
        ...
        >
          ...
          <span>{{ isSelected(dow, period.n) ? 'Có học' : 'Không học' }}</span>
        </button>
      </template>
    </div>
    ...
    <div class="mobile-schedule">
      ...
      <div class="mobile-periods" role="tabpanel" :aria-label="days[activeDay]">
        ...
        >
          ...
          <span class="mobile-state"><Check v-if="isSelected(activeDay, period.n)" aria-hidden="true" />{{ isSelected(activeDay, period.n) ? 'Bật' : 'Tắt' }}</span>
        </button>
      </div>
    </div>
  </div>
