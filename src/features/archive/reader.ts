 ...
 * do not fit comfortably in memory, so nothing inflates the whole archive:
 * the small JSON members are read up front and each image is inflated only when
 * it is displayed, then released (EC-710/AC-727/AC-728).
 */
...
export async function openArchive(
  bytes: Uint8Array,
  options: { makeObjectUrl?: (blob: Blob) => string; revokeObjectUrl?: (url: string) => void } = {},
  ...
  // AC-715/AC-720. Text members are checked now; images are checked as they are
  // opened, so opening the archive does not inflate every picture at once.
  ...
  return {
    ...
    async openMedia(path: string) {
      ...
      const member = await inflateOne(bytes, path)
