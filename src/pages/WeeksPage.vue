...
function loadDrafts(source=state.value){if(!source)return;const next=buildWeekDrafts(source.weeks);drafts.value=next;initialDrafts.value=structuredClone(next);if(!selectedDraftId.value||!next.some(item=>item.id===selectedDraftId.value))selectedDraftId.value=chooseDefaultDraft(next);status.value='idle';statusMessage.value='';dirtyEditor.markClean()}
