...
.hero-card {
  ...
  min-height: 0;
  overflow: hidden;
  border: 1px solid color-mix(in srgb, var(--color-sky) 20%, var(--border));
  border-radius: 24px;
  background: linear-gradient(145deg, var(--wash-sky), var(--wash-sun));
  box-shadow: 0 14px 34px color-mix(in srgb, var(--color-sky) 12%, transparent);
}
...
.login-panel {
  ...
  background: linear-gradient(
    160deg,
    var(--surface-raised),
    color-mix(in srgb, var(--wash-pink) 34%, var(--surface-raised))
  );
}
...
.submit { width: 100%; min-height: 50px; }
...
@media (max-width: 980px) {
  ...
  .login-visual { grid-template-rows: auto auto 250px auto; }
