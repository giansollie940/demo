...
import { assignTeacher, createClass, createSchoolYear, createTeacher, deleteClass, deleteTeacher, setActiveSchoolYear, updateClass, updateTeacher, useAdminDirectory, type AdminMutationRuntime } from '../features/admin/admin-directory'
...
const data=computed(()=>directory.data.value??{schoolYears:[],classes:[],teachers:[],assignments:[]})
...
const selectedYear=computed(()=>data.value.schoolYears.find(item=>item.id===selectedYearId.value)??data.value.schoolYears.find(item=>item.active)??null)
...
function openClass(id:string){context.selectClass(id);void auth.reload(id,context.selectedSchoolYearId).then(()=>{context.hydrate(auth.legacyState);void router.push('/students')})}
...
<template>
  <div class="page-stack admin-page">
    ...
    <template v-if="tab==='overview'">
      <section class="summary"><AppCard padding="lg"><span><CalendarDays/>Năm học</span><b>{{ data.schoolYears.length }}</b></AppCard><AppCard padding="lg"><span><Building2/>Lớp hoạt động</span><b>{{ activeClasses.length }}</b></AppCard><AppCard padding="lg"><span><UserCog/>Giáo viên hoạt động</span><b>{{ activeTeachers.length }}</b></AppCard><AppCard padding="lg"><span><ShieldCheck/>Phân quyền</span><b>{{ activeAssignments.length }}</b></AppCard></section>
      <AppCard padding="lg" class="overview-copy"><h2>Ngữ cảnh hiện tại: {{ selectedYear?.name||'Chưa có năm học' }}</h2><p>Năm học quyết định danh sách lớp và tuần hiển thị. Chuyển năm học ở bong bóng trên đầu trang để xem dữ liệu lịch sử hoặc chuẩn bị năm học mới.</p></AppCard>
    </template>

    <template v-else-if="tab==='years'">
      <div class="section-actions"><div><h2>Năm học</h2><p>{{ data.schoolYears.length }} năm học. Chỉ một năm được đặt là đang hoạt động.</p></div><AppButton @click="showYearForm=!showYearForm"><Plus/>Tạo năm học</AppButton></div>
      ...
      <section class="year-grid"><AdminSchoolYearCard v-for="item in data.schoolYears" :key="item.id" :item="item" :busy="busyKey===`year:${item.id}`" @activate="activateYear"/></section>
    </template>
    ...
    <template v-else-if="tab==='classes'">
      ...
      <section class="class-grid"><AdminClassCard v-for="item in yearClasses" :key="item.id" :item="item" :teachers="assignedTeachers(item.id)" @open="openClass(item.id)" @edit="editClass(item.id)" @toggle="toggleClass(item.id)" @delete="removeClass(item.id)"/></section>
    </template>
...
<style scoped>
.admin-page{max-width:1560px;margin:0 auto}.admin-header,.section-actions{display:flex;justify-content:space-between;align-items:flex-start;gap:16px}.admin-header h1{margin:8px 0;font-size:clamp(2rem,4vw,3rem)}.admin-header p,.section-actions p,.overview-copy p{margin:0;color:var(--text-muted)}.page-context{display:flex;align-items:center;gap:7px;color:var(--color-primary);font-size:.75rem;font-weight:900;letter-spacing:.04em}.page-context svg,.admin-header :deep(svg),.section-actions :deep(svg){width:17px}.summary{display:grid;grid-template-columns:repeat(4,1fr);gap:11px}.summary :deep(.app-card){display:flex;align-items:center;justify-content:space-between}.summary span{display:flex;align-items:center;gap:7px;color:var(--text-muted);font-weight:800}.summary svg{width:18px}.summary b{font-size:1.8rem}.overview-copy h2,.section-actions h2{margin:0 0 5px}.year-grid,.class-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:11px}.teacher-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:11px}.quick-form{display:grid;grid-template-columns:1fr 2fr auto;gap:10px;align-items:end}.quick-form.year-form{grid-template-columns:1.1fr 1fr 1fr 1.3fr auto}.quick-form.teacher-form{grid-template-columns:1fr 1.5fr 1.3fr auto}.quick-form label{display:grid;gap:5px;font-size:.78rem;font-weight:800;color:var(--text-muted)}.quick-form input{min-height:44px;border:1px solid var(--border);border-radius:11px;background:var(--surface);color:var(--text);padding:8px 10px}.quick-form .check-field{display:flex;align-items:center;gap:8px;min-height:44px;padding:8px 10px;border:1px solid var(--border);border-radius:11px;background:color-mix(in srgb,var(--wash-cream) 48%,var(--surface));color:var(--text)}.quick-form .check-field input{min-height:0;width:17px;height:17px}@media(max-width:1180px){.summary{grid-template-columns:repeat(2,1fr)}.quick-form.year-form{grid-template-columns:1fr 1fr}.teacher-grid{grid-template-columns:repeat(2,1fr)}.quick-form,.quick-form.teacher-form{grid-template-columns:1fr 1fr}.quick-form :deep(.app-button){width:100%}}@media(max-width:720px){.admin-header,.section-actions{flex-direction:column}.summary,.year-grid,.class-grid,.teacher-grid{grid-template-columns:1fr}.quick-form,.quick-form.year-form,.quick-form.teacher-form{grid-template-columns:1fr}.admin-header :deep(.app-button),.section-actions :deep(.app-button){width:100%}}
</style>
