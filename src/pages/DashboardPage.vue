...
/* ══════════════════════════════════════════════════
   Hero card — CSS Grid 2 cột cân đối
   ══════════════════════════════════════════════════ */
.dashboard-hero {
  min-height: 132px;
  display: grid;
  ...
  position: relative;
  background:
    radial-gradient(circle at 76% 24%, color-mix(in srgb, var(--wash-sky) 72%, transparent), transparent 31%),
    radial-gradient(circle at 8% 0%, color-mix(in srgb, var(--wash-pink) 72%, transparent), transparent 36%),
    linear-gradient(135deg, color-mix(in srgb, var(--surface) 98%, transparent), color-mix(in srgb, var(--wash-violet) 58%, var(--surface)));
  border-radius: 24px;
}
.dashboard-hero::after {
  content: "";
  position: absolute;
  right: -82px; bottom: -122px;
...
/* ── Hero text — grid tự cân đối, không cần max-width / margin-left ── */
.hero-copy {
  position: relative;
  z-index: 2;
  display: grid;
...
.hero-copy h1 {
  margin: 0;
  font-size: clamp(1.95rem, 2.4vw, 2.25rem);
  line-height: 1.08;
  color: var(--text);
...
/* ── Hero illustration — in-flow, order:-1 để nằm bên trái ── */
.hero-illustration {
  position: relative;
  width: 100%;
  height: 100%;
...
.hero-illustration img {
  ...
  /* Mask fade bên PHẢI (hướng sang khung chữ) */
  -webkit-mask-image: linear-gradient(90deg, #000 60%, rgba(0,0,0,.22) 88%, transparent 100%);
          mask-image: linear-gradient(90deg, #000 60%, rgba(0,0,0,.22) 88%, transparent 100%);
}

/* ══════════════════════════════════════════════════
   Role sections & stat cards
   ══════════════════════════════════════════════════ */
.role-section { display: grid; gap: 10px }
.role-heading h2 { margin: 3px 0 0; font-size: 1.18rem }
...
.metrics :deep(.card) {
  ...
  align-items: center;
  padding: 18px;
  background: linear-gradient(145deg, color-mix(in srgb, var(--surface) 96%, transparent), color-mix(in srgb, var(--metric-wash) 96%, transparent));
  border-color: color-mix(in srgb, var(--metric-accent) 18%, var(--border));
  border-radius: 21px;
  box-shadow: 0 7px 18px color-mix(in srgb, var(--metric-accent) 6%, transparent);
}
.metrics :deep(.card:nth-child(1)) { --metric-accent: var(--color-sky);   --metric-wash: var(--wash-sky) }
.metrics :deep(.card:nth-child(2)) { --metric-accent: var(--color-mint);  --metric-wash: var(--wash-mint) }
.metrics :deep(.card:nth-child(3)) { --metric-accent: var(--color-sun);   --metric-wash: var(--wash-sun) }
.metrics :deep(.card:nth-child(4)) { --metric-accent: var(--color-lilac); --metric-wash: var(--wash-violet) }
.metrics :deep(svg) {
  grid-row: 1 / 3;
  ...
  background: color-mix(in srgb, var(--metric-accent) 13%, var(--surface));
  box-sizing: content-box;
}
.metrics b    { font-size: 1.82rem; line-height: 1.02; color: var(--text) }
.metrics span { color: var(--text-muted); font-size: .82rem; font-weight: 760 }
...
.dashboard-main-grid {
  display: grid;
  grid-template-columns: minmax(0, 820px) minmax(0, 454px);
  gap: 18px;
}
.work-panel,
.overview-panel {
  ...
  box-shadow: 0 8px 22px rgb(79 55 73 / .055);
}
.work-panel.pad-lg,
.overview-panel.pad-lg { padding: 22px }
.panel-heading h2 { margin: 2px 0 4px; font-size: 1.28rem; line-height: 1.25 }
.panel-heading p   { margin: 0; color: var(--text-muted); font-size: var(--font-size-ui-min); line-height: 1.45 }
...
.task-row b      { font-size: .86rem; line-height: 1.35 }
.task-row small  { color: var(--text-muted); font-size: var(--font-size-ui-min); line-height: 1.4 }
.task-row strong { font-size: .88rem }

.task-dot,
.legend-dot     { display: block; width: 10px; height: 10px; border-radius: 50% }
.task-warning   { background: var(--color-sun) }
.task-coral     { background: var(--color-coral) }
.task-violet    { background: var(--color-lilac) }
.issue-link     { display: inline-flex; margin-top: 10px; color: var(--color-warning); font-size: var(--font-size-ui-min); font-weight: 850 }
...
.progress-donut {
  ...
  display: grid;
  place-items: center;
  background: conic-gradient(var(--color-mint) var(--progress), color-mix(in srgb, var(--surface-soft) 82%, var(--border)) 0);
  position: relative;
  margin: auto;
}
...
.progress-donut strong { font-size: 1.58rem; color: var(--color-mint) }
.progress-donut span   { color: var(--text-muted); font-size: var(--font-size-ui-min); font-weight: 700 }

.overview-legend      { display: grid; gap: 11px }
.overview-legend div  { display: grid; grid-template-columns: auto 1fr auto; align-items: center; gap: 8px; color: var(--text-muted); font-size: var(--font-size-ui-min) }
.overview-legend span { font-size: var(--font-size-ui-min) }
.overview-legend b    { color: var(--text); font-size: var(--font-size-ui-min) }
.legend-dot.done      { background: var(--color-mint) }
.legend-dot.approved  { background: var(--color-sky) }
.legend-dot.pending   { background: var(--color-coral) }
...
.daily-quote :deep(.card) {
  ...
  background: linear-gradient(120deg,
    color-mix(in srgb, var(--wash-sky) 90%, transparent),
    color-mix(in srgb, var(--wash-violet) 88%, transparent) 48%,
    color-mix(in srgb, var(--wash-pink) 88%, transparent));
  border-color: color-mix(in srgb, var(--color-lilac) 18%, var(--border));
}
...
.daily-quote cite { color: var(--text-muted); font-style: normal }

/* ══════════════════════════════════════════════════
   Responsive
   ══════════════════════════════════════════════════ */
@media (max-width: 1400px) {
  .dashboard-page { max-width: 1220px }
  .dashboard-main-grid { grid-template-columns: minmax(0, 1.55fr) minmax(320px, .85fr) }
}
...
@media (max-width: 1100px) {
  ...
  .work-panel,
  .overview-panel { height: auto; min-height: 300px }

  /* Hero chuyển sang 1 cột, hình nằm dưới chữ */
  .dashboard-hero {
  ...
  .overview-body { grid-template-columns: minmax(170px, 220px) 1fr }
}

@media (max-width: 640px) {
  .dashboard-stat-grid { grid-template-columns: 1fr }
  .dashboard-hero { padding: 22px 20px 0 }
  .hero-illustration {
    height: 138px;
    margin-inline: -20px;
    width: calc(100% + 40px);
  }
  .hero-illustration img { height: 138px }
  .overview-body { grid-template-columns: 1fr }
  .progress-donut { width: 150px }
}
</style>
