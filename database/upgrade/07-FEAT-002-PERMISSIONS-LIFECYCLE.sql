-- FEAT-002 additive upgrade. Apply after 05 and 06; no production deployment by this task.
begin;
alter table public.homework_settings add column semantic_duplicate_enabled boolean not null default true;
alter table public.homework_settings drop constraint homework_settings_check;
alter table public.homework_settings add constraint homework_settings_ai_thresholds check(pending_threshold between 0 and 99 and reject_threshold>pending_threshold and reject_threshold<=100);
create table public.homework_tombstones (
 notice_id uuid primary key, class_id uuid not null references public.classes(id),
 author_id uuid not null references public.profiles(id),subject_id uuid references public.class_subjects(id),
 english_group_id uuid references public.english_groups(id),original_created_at timestamptz not null,
 original_published_at timestamptz,final_status text not null check(final_status in('published','pending_duplicate_review','duplicate_rejected','replaced')),
 soft_deleted_at timestamptz not null,soft_deleted_by uuid not null references public.profiles(id),soft_delete_reason text,
 hard_deleted_at timestamptz not null default now(),hard_deleted_by uuid not null references public.profiles(id),
 hard_delete_reason text not null check(length(trim(hard_delete_reason)) between 1 and 500),
 was_duplicate boolean not null,last_decision text check(last_decision in('keep_existing','replace_existing','keep_both')),
 decided_at timestamptz,decided_by uuid references public.profiles(id)
);
alter table public.homework_tombstones enable row level security;
revoke all on public.homework_tombstones from public,anon,authenticated;
alter table public.homework_notices add column duplicate_tombstone_id uuid references public.homework_tombstones(notice_id);
alter table public.homework_duplicate_reviews add column candidate_tombstone_id uuid references public.homework_tombstones(notice_id);
create function homework_private.immutable_tombstone() returns trigger language plpgsql set search_path=pg_catalog,public as $$
begin raise exception 'Hard-delete tombstone is immutable';end$$;
create trigger homework_tombstone_immutable before update or delete or truncate on public.homework_tombstones for each statement execute function homework_private.immutable_tombstone();
create function homework_private.ai_config(st public.homework_settings) returns jsonb language sql immutable set search_path=pg_catalog,public as $$
 select jsonb_build_object('semantic_duplicate_enabled',st.semantic_duplicate_enabled,'duplicate_review_threshold',st.pending_threshold,'duplicate_auto_threshold',st.reject_threshold)
$$;
create function homework_private.actionable(n public.homework_notices) returns boolean language sql stable set search_path=pg_catalog,public as $$
 select n.status='pending_duplicate_review' and n.deleted_at is null and not exists(select 1 from public.homework_duplicate_reviews rv where rv.notice_id=n.id and rv.revision=n.revision and rv.decision is not null)
$$;
create function homework_private.notification_visible(hn public.homework_notifications,a public.profiles) returns boolean language sql stable set search_path=pg_catalog,public as $$
 select (hn.notice_id is null or exists(select 1 from public.homework_notices n where n.id=hn.notice_id and homework_private.visible(n,a)
 and (n.status='published' or a.role::text in('teacher','admin'))
 and (hn.kind not in('pending','pending_duplicate_review') or homework_private.actionable(n))))
 and (hn.kind<>'backlog' or exists(select 1 from public.homework_backlog_state b where b.class_id=hn.class_id and b.active))
$$;
create function homework_private.hard_delete(c uuid,a public.profiles,p jsonb) returns jsonb language plpgsql set search_path=pg_catalog,public as $$
...
begin
 ...
 if n.status<>'deleted' or n.deleted_at is null or n.deleted_by is null then raise exception 'Chỉ xóa vĩnh viễn bài đã soft-delete';end if;
 ...
 insert into public.homework_tombstones(notice_id,class_id,author_id,subject_id,english_group_id,original_created_at,original_published_at,final_status,soft_deleted_at,soft_deleted_by,soft_delete_reason,hard_deleted_by,hard_delete_reason,was_duplicate,last_decision,decided_at,decided_by)
 values(n.id,c,n.author_id,n.subject_id,n.english_group_id,n.created_at,n.published_at,n.previous_status,n.deleted_at,n.deleted_by,n.delete_reason,a.id,reason_text,
...
create or replace function homework_private.card(n public.homework_notices,p_user uuid) returns jsonb language sql stable set search_path=pg_catalog,public as $$
 ...
 'subject',s.name,'icon',s.icon,'english_group',g.name,'duplicate_of',n.duplicate_of,'duplicate_tombstone_id',n.duplicate_tombstone_id,
 ...
 and exists(select 1 from public.profiles actor where actor.id=p_user and (actor.id=n.author_id or (actor.role::text='teacher' and p.role::text in('student','monitor')))))
...
create or replace function homework_private.backlog(c uuid) returns void language plpgsql set search_path=pg_catalog,public as $$
declare b public.homework_backlog_state; qty int;begin
 ...
 select count(*) into qty from public.homework_notices where class_id=c and homework_private.actionable(homework_notices) and pending_since<=now()-interval '24 hours';
...
create or replace function public.homework_api(p_action text,p_data jsonb default '{}'::jsonb) returns jsonb
...
begin
 ...
 if a.role::text in('monitor','teacher') then
 ...
 where x.class_id=c and homework_private.actionable(x)),'[]'));
 end if;
 if a.role::text in('teacher','admin') then
 r:=r||jsonb_build_object('settings',jsonb_build_object('seed_threshold',st.seed_threshold),
 ...
 r:=r||jsonb_build_object('ai_settings',homework_private.ai_config(st),
 'review_history',coalesce((select jsonb_agg(homework_private.card(x,a.id)||jsonb_build_object('candidate',homework_private.card(o,a.id),'score',rv.score,'reason',rv.reason,'decision',rv.decision) order by x.updated_at desc)
 from public.homework_notices x left join public.homework_notices o on o.id=x.duplicate_of left join public.homework_duplicate_reviews rv on rv.notice_id=x.id and rv.revision=x.revision
 where x.class_id=c and x.status='duplicate_rejected'),'[]'));
 end if;
 r:=r||jsonb_build_object('history_markers',coalesce((select jsonb_agg(jsonb_build_object('notice_id',t.notice_id,'marker','[Đã xóa vĩnh viễn]','original_created_at',t.original_created_at,'hard_deleted_at',t.hard_deleted_at) order by t.original_created_at desc)from public.homework_tombstones t where t.class_id=c and t.author_id=a.id),'[]'),
 'tombstones',case when a.role::text='admin' then coalesce((select jsonb_agg(to_jsonb(t) order by t.hard_deleted_at desc)from public.homework_tombstones t where t.class_id=c),'[]')
 when a.role::text='teacher' then coalesce((select jsonb_agg(jsonb_build_object('notice_id',t.notice_id,'event','Admin đã xóa vĩnh viễn notice','hard_deleted_at',t.hard_deleted_at) order by t.hard_deleted_at desc)from public.homework_tombstones t where t.class_id=c),'[]')else '[]'::jsonb end);
 ...
 'health',jsonb_build_object('total',(select count(*)from public.homework_notices where class_id=c),'pending',(select count(*)from public.homework_notices where class_id=c and homework_private.actionable(homework_notices))));end if;
 return r;
 ...
 perform homework_private.audit(c,null,a.id,'settings',jsonb_build_object('seed_threshold',st.seed_threshold),p_data);return '{"ok":true}';
 elsif p_action='ai_settings' then
 if a.role::text<>'teacher' then raise exception 'Chỉ Teacher được đổi cấu hình AI' using errcode='42501';end if;
 if exists(select 1 from jsonb_object_keys(p_data) k where k not in('class_id','semantic_duplicate_enabled','duplicate_review_threshold','duplicate_auto_threshold'))
 or jsonb_typeof(p_data->'semantic_duplicate_enabled') is distinct from 'boolean'
 or jsonb_typeof(p_data->'duplicate_review_threshold') is distinct from 'number'
 or jsonb_typeof(p_data->'duplicate_auto_threshold') is distinct from 'number'
 then raise exception 'Cấu hình AI không hợp lệ';end if;
 if (p_data->>'duplicate_review_threshold')::numeric<>trunc((p_data->>'duplicate_review_threshold')::numeric)
 or (p_data->>'duplicate_auto_threshold')::numeric<>trunc((p_data->>'duplicate_auto_threshold')::numeric)then raise exception 'Ngưỡng AI phải là số nguyên';end if;
 before_json:=homework_private.ai_config(st);
 update public.homework_settings set semantic_duplicate_enabled=(p_data->>'semantic_duplicate_enabled')::boolean,
 pending_threshold=(p_data->>'duplicate_review_threshold')::numeric::int,reject_threshold=(p_data->>'duplicate_auto_threshold')::numeric::int where class_id=c returning * into st;
 perform homework_private.audit(c,null,a.id,'ai_settings',before_json,homework_private.ai_config(st));return '{"ok":true}';
 ...
 if a.role::text<>'teacher' then raise exception 'Chỉ giáo viên được cấu hình' using errcode='42501';end if;
 ...
 revision=revision+1,updated_at=now(),pending_since=case when pending then now()else pending_since end,status=case when pending then 'pending_duplicate_review'else status end,duplicate_of=case when pending then null else duplicate_of end,duplicate_tombstone_id=case when pending then null else duplicate_tombstone_id end
 ...
 if not exists(select 1 from public.homework_duplicate_reviews rv where rv.notice_id=n.id and rv.revision=n.revision and rv.score is not null and rv.candidate_id=n.duplicate_of)then raise exception 'Chưa có kết quả so sánh hợp lệ cho phiên bản hiện tại';end if;
...
create or replace function public.homework_ai(p_action text,p_data jsonb)returns jsonb language plpgsql security definer set search_path=pg_catalog,public as $$
declare n public.homework_notices; cand jsonb; score numeric; candidate uuid; st public.homework_settings; final_state text; old jsonb;begin
 ...
 if n.id is null then return '{"done":true,"deleted":true}';end if;
 ...
 return jsonb_build_object('notice',to_jsonb(n),'previous',old,'candidates',cand,'fingerprint',md5(cand::text||homework_private.ai_config(st)::text),'settings',homework_private.ai_config(st));
 elsif p_action='finish' then
 if n.revision is distinct from (p_data->>'revision')::int or md5(cand::text||homework_private.ai_config(st)::text) is distinct from (p_data->>'fingerprint') then return '{"stale":true}';end if;
 ...
 if score is null or score<0 or score>100 or (score<>0 and candidate is null)then raise exception 'Kết quả AI không hợp lệ';end if;
 ...
 final_state:=case when candidate is null then 'published' when score>=st.reject_threshold then 'duplicate_rejected'when score>=st.pending_threshold then 'pending_duplicate_review'else 'published'end;
 update public.homework_notices set status=final_state,duplicate_tombstone_id=null,duplicate_of=case when score>=st.pending_threshold then candidate else null end,
...
revoke all on function public.homework_ai(text,jsonb) from public,anon,authenticated;
grant execute on function public.homework_ai(text,jsonb) to service_role;
commit;
