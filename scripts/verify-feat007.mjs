// Local-only verification for FEAT-007. No remote database, deployment or Git mutations.
import {spawn} from 'node:child_process';
import {mkdir,writeFile,copyFile,access,readFile} from 'node:fs/promises';
import {fileURLToPath} from 'node:url';
const root=fileURLToPath(new URL('../',import.meta.url));
const evidence=new URL('../docs/feat-007/evidence/',import.meta.url);
await mkdir(evidence,{recursive:true});
const archive=new URL('previous-'+Date.now()+'/',evidence);
await mkdir(archive,{recursive:true});
for(const name of ['archive-database.log','archive-ui.log','storage-database.log','storage-database-on-12.log','storage-ui.log','media-database.log','media-database-on-12.log','database-on-12.log','media-ui.log','database.log','legacy-database.log','feat004-database.log','static.log','ui.log','typecheck.log','build.log','verification-results.json']){
