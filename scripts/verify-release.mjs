...
for (const name of functions) {
  ...
  if (name==='audit-log') {
    if (!/AUDIT_LIST_CONTRACT_VERSION\s*=\s*2/.test(indexText.stdout)) {
      fail('audit-log ZIP does not contain the current AUDIT_LIST_CONTRACT_VERSION=2 marker')
    }
    const auditSource = fs.readFileSync(path.join(root, 'supabase', 'functions', 'audit-log', 'index.ts'), 'utf8')
    if (indexText.stdout !== auditSource) fail('audit-log source and ZIP source differ')
  }
