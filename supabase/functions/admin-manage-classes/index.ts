import {requireClassGrade,rejectGradeEdit} from "../_shared/class-grade.ts";
...
async function loadClass(admin:any,classId:string){
  const {data,error}=await admin.from("classes")
    .select("id,school_year_id,code,name,grade,active,created_at,updated_at")
  ...
  try{
    ...
    if(action==="list"){
      const [classes,teachers,assignments,schoolYears,weeks,periods,timetableTemplates,timetableVersions,timetableAssignments]=await Promise.all([
        admin.from("classes").select("id,school_year_id,code,name,grade,active,created_at,updated_at").order("code"),
    ...
    if(action==="create_class"){
      const grade=requireClassGrade(body?.grade);
      ...
      const {data,error}=await admin.from("classes")
        .insert({school_year_id:schoolYearId,code,name,grade,active:true,created_by:actor.id})
        .select().single();
      if(error)throw error;
      ...
      try{
        ...
        if(weeks?.length){
          const {error:classWeeksError}=await admin.from("class_weeks").insert(weeks.map((w:any)=>({
            ...
            week_id:w.id,
            status:w.status,
            deadline_mode:w.deadline_mode==="specific"&&w.registration_deadline?"specific":"per_session_20",
            registration_deadline:w.deadline_mode==="specific"&&w.registration_deadline?w.registration_deadline:null,
            note:w.note,
            updated_by:actor.id
          })));
