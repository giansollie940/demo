...
const AUDIT_LIST_CONTRACT_VERSION = 2;
...
function auditSchemaFailure(error: any) {
  ...
  if (code === "42703" || code === "PGRST204" || /audit_logs|class_id|source/i.test(message)) {
    return Object.assign(
      new Error("Schema Nhật ký hệ thống chưa sẵn sàng. Hãy chạy SQL upgrade/VERIFY của phiên bản đang dùng."),
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if(action==="list"){
      ...
      return json(req,200,{ok:true,contract:"audit-list",contractVersion:AUDIT_LIST_CONTRACT_VERSION,logs,limit});
    }
